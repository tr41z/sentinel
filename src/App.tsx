...
function App() {
  ...
  const fetchStats = async () => {
    try {
      // Fetch Sniffer stats
      const snifferRes = await fetch(
        "http://localhost:8080/api/v1/sniffer/health"
      );
      if (snifferRes.ok) {
    ...
    try {
      ...
      const flaggedRes = await fetch(
        "http://localhost:8080/api/v1/flagged_ips"
      );
      ...
      if (threatRes.ok && flaggedRes.ok) {
        ...
        setAiStats((prev) => ({
          ...
          ips_flagged: flaggedData.data?.length || 0, 
          threatCount: Math.max(
            prev?.threatCount ?? 0,
            threatsData.data?.length || 0 
          ),
        }));
      } else {
  ...
  const bandwidth =
    totalDuration > 0 ? (totalBytes * 8.0) / (totalDuration * 1000) : 0;
